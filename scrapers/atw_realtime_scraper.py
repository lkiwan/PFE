...
-----------
  (no cmd)   Runs default flow without writing intraday/orderbook CSVs:
             `snapshot` only (appends one timestamped Medias24 row per run
             to data/historical/ATW_bourse_casa_{YYYY-MM-DD}.csv).
...
State file: data/scrapers/atw_realtime_state.json
...
EOD_CSV = DATA_DIR / "ATW_bourse_casa_full.csv"  # Legacy fallback for reads only.
...
# Intraday day log schema with exact scrape metadata.
DAY_EOD_FIELDS = FULL_EOD_FIELDS + ["scraped_at", "cotation", "source"]
...
def _eod_day_csv(day: str) -> Path:
    return DATA_DIR / f"ATW_bourse_casa_{day}.csv"


def _snapshot_to_daily_row(day: str, snap: Snapshot, source: str) -> Dict[str, Any]:
    return {
        "Séance": day,
        "Instrument": TICKER,
        "Ticker": TICKER,
        "Ouverture": snap.open or "",
        "Dernier Cours": snap.last_price or "",
        "+haut du jour": snap.high or "",
        "+bas du jour": snap.low or "",
        "Nombre de titres échangés": snap.shares_traded or "0",
        "Volume des échanges": snap.value_traded_mad or "0",
        "Nombre de transactions": snap.num_trades or "0",
        "Capitalisation": snap.market_cap or "",
        "scraped_at": snap.timestamp,
        "cotation": snap.cotation or "",
        "source": source,
    }
...
def _append_technicals_snapshot(
    ...
    path = DATA_DIR / f"ATW_technicals_{day}.json"
    entry = {
    ...
    if path.exists():
        try:
            ...
            if isinstance(payload, list):
                ...
                # Migrate legacy single-object format to list history.
...
def cmd_snapshot(args) -> int:
    ...
    if market == "CLOSED" and last_ts and not args.force:
        try:
            ...
            if same_day:
                ...
                if save_raw:
                    write_intraday(cached, day)
                _append_row(_eod_day_csv(day), DAY_EOD_FIELDS, _snapshot_to_daily_row(day, cached, "medias24_cached_snapshot"))
                _log_summary(cached)
                ...
                _sync_bourse_daily_to_full(day)
                return 0
        except ValueError:
            pass
    ...
    if save_raw:
        write_intraday(snap, day)
        write_orderbook(ob, day)
    _append_row(_eod_day_csv(day), DAY_EOD_FIELDS, _snapshot_to_daily_row(day, snap, "medias24_live_snapshot"))
...
def _sync_bourse_daily_to_full(day: str) -> None:
    """Best-effort sync: write official Casablanca Bourse daily row to full CSV."""
    ...
    logger.info("Official Bourse daily row saved for %s -> %s", day, EOD_CSV.name)
...
def cmd_finalize(args) -> int:
    ...
    eod_day_path = _eod_day_csv(day)

    if day in finalized and not args.force:
        if _csv_has_day(EOD_CSV, day) or _csv_has_day(eod_day_path, day):
    ...
    target_path = EOD_CSV if eod.get("source") == "casablanca_bourse_daily_api" else eod_day_path
    target_fields = FULL_EOD_FIELDS if target_path == EOD_CSV else DAY_EOD_FIELDS
    if not args.force and _csv_has_day(target_path, day):
        logger.info("EOD row for %s already in %s — marking finalized, skip append", day, target_path.name)
        finalized.add(day)
...
def _try_intraday_finalize(day: str) -> Optional[Dict[str, str]]:
    """Fall back to aggregating from the intraday CSV."""
    intraday_path = DATA_DIR / f"ATW_intraday_{day}.csv"
    if not intraday_path.exists():
        logger.warning("no intraday file for %s (%s)", day, intraday_path)
        return None

    rows: List[Dict[str, str]] = []
    with open(intraday_path, "r", encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            rows.append(r)
    if not rows:
        logger.warning("intraday file empty for %s", day)
        return None
...
def main() -> int:
    ...
    if not hasattr(args, "func"):
        logger.info("No cmd provided — running snapshot only (daily Medias24 CSV append)")
